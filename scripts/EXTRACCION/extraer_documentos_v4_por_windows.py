...
def extraer_imagen(ruta: Path) -> dict:
    """Aplica OCR sobre imágenes, incluyendo AVIF mediante Pillow.

    Para AVIF se carga explícitamente el plugin de Pillow y se normaliza la
    imagen a RGB. Esto evita pasar un AVIF directamente a Tesseract, que puede
    rechazar el formato aunque Pillow sí pueda decodificarlo.
    """
    try:
        import pytesseract
        from PIL import Image
    ...
    # El plugin se registra en Pillow al importarlo. No lo hacemos silencioso:
    # si falta y el archivo es AVIF, devolvemos un error explicativo.
    if ruta.suffix.lower() == ".avif":
        try:
            ...
            return {
                "texto": "",
                "estado": "error",
                "detalle": f"AVIF requiere pillow-avif-plugin: {exc}"[:300],
            }
    ...
    try:
        ...
        return {
            ...
            "detalle": f"Tesseract no disponible en {TESSERACT_CMD}: {exc}"[:300],
        }

    img = None
    normalizada = None
    try:
        # Cargamos la imagen con Pillow. Para AVIF/WEBP/GIF/TIFF y otros
        # formatos, la normalizamos a RGB antes de enviarla a Tesseract.
        img = Image.open(ruta)
        img.load()
        normalizada = img.convert("RGB") if img.mode not in ("RGB", "L") else img.copy()

        # Tesseract recibe un objeto Pillow normalizado; no recibe el AVIF
        # original. Esto elimina el error "Unsupported image format/type".
        texto = pytesseract.image_to_string(normalizada, lang=OCR_LANG)
    except Exception as exc:
        return {"texto": "", "estado": "error", "detalle": f"OCR imagen: {exc}"[:300]}
    finally:
        for objeto in (normalizada, img):
            try:
...
def limite_memoria_worker(gib: float) -> None:
    """Aplica limite de memoria cuando el sistema operativo lo permite.

    ``resource``/RLIMIT_AS es una API de Unix y no existe en Windows.
    En Windows el control de memoria se hace mediante el reciclado de workers
    (max_tasks_per_child), especialmente en la pasada de archivos grandes.
    """
    if os.name == "nt":
        # Windows: no importar ``resource``. Si lo hicieramos, todos los
        # workers moririan en el inicializador antes de procesar un archivo.
        return

    try:
        import resource
    except ImportError:
        log.warning("modulo resource no disponible; se omite limite de memoria del worker")
        return
